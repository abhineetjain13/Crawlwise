...
- browser fetch now has separate `patchright` and `real_chrome` lanes for protected targets. `patchright` is the preferred patched-Chromium lane when installed, but it defaults to context-options-only identity shaping with the legacy JS spoof bundle disabled unless `CRAWLER_RUNTIME_BROWSER_PATCHRIGHT_USE_LEGACY_INIT_SCRIPT=true` is set for experiments. When `C:\Program Files\Google\Chrome\Application\chrome.exe` (or `CRAWLER_RUNTIME_BROWSER_REAL_CHROME_EXECUTABLE_PATH`) is available, the `real_chrome` fallback can launch native local Chrome headful and use a native context with stealth disabled by default, while probe-mode runs can still inject the full fingerprint scripts
