...
| Labeled product option groups are real variants | `detail_extractor.py`, `extract/shared_variant_logic.py` | DOM variant recovery must recognize labeled select/radio/checkbox option groups generically, normalize common aliases, and carry per-option stock/availability where present without requiring site-specific axis names | Sites with visible product-option chips such as `Weight`, `Flavour`, `Shade`, or `Storage` keep losing `variant_axes`, `variants`, and selected availability even though the DOM has them |
