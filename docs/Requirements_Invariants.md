...
| **Platform Adapter** | A code-backed, version-controlled extraction module for a specific platform or site family (e.g. Workday, Shopify, iCIMS). The authoritative source for field mappings on that platform. |
| **Extraction Hierarchy** | The strict ordered chain of extraction sources: Platform Adapter → JSON API payload → JSON-LD → hydrated JS state (`__NEXT_DATA__` / `__NUXT_DATA__`) → DOM selector → LLM fallback. First valid hit wins; no further sources are consulted. |
| **LLM Cleanup Layer** | Non-deterministic normalisation pass over raw multi-source data; output is user-confirmed before storage |
...
| **Acquisition hardening** | Generic transport-layer resilience controls such as retries, browser fallback, header normalisation, challenge detection, proxy rotation, and diagnostics capture that apply consistently across production targets rather than being tailored to one site |
...
- Login returns a JWT; token version is stored server-side to support immediate revocation on deactivation (see INV-AUTH-02)
...
| Intelligence | LLM-generated plain summary of the crawled dataset |
...
**Pre-run Preview**
- Show: detected output columns and a sample value if a preview fetch is available
- User can add or remove requested fields before confirming

**Field Configuration**
- User specifies which fields they want extracted (e.g. `title`, `price`, `returns`, `specifications`)
- The system resolves each field through the Extraction Hierarchy (see §1 Glossary and INV-EXTRACT-01)
- Users do not provide XPath expressions or CSS selectors; field extraction strategy is determined by code, not runtime configuration

**LLM Cleanup Layer**
- After raw extraction, data is passed to the configured LLM as a fallback for fields not resolved by earlier hierarchy steps
- The LLM produces cleaned, normalised column suggestions (name + value per record)
...
| Intelligence | LLM-generated plain summary and data quality notes |
| Logs | Real-time job log stream |

**Downloads**
- CSV (from structured records)
- JSON
...
**Purpose:** Manage the LLM provider used by the platform for field extraction fallback and the data cleanup layer.
...
> **Note (v2.0):** LLM is used as an extraction fallback and for the cleanup layer. It is not used for XPath suggestion or selector generation — those features have been removed.
...
- **INV-AUTH-02:** A deactivated user's token is rejected immediately, even if the token has not expired. Token version on the User model is the enforcement mechanism; incrementing it invalidates all previously issued tokens for that user.
...
- **INV-JOB-03:** A job in `COMPLETED`, `KILLED`, or `FAILED` state cannot be re-paused, re-killed, or re-resumed. A re-run creates a new job with a new Run ID.
...
- **INV-LLM-05:** LLM calls do not retry on rate-limit errors (HTTP 429). The failure is surfaced to the user immediately.
...
- **INV-EXTRACT-01:** Field extraction follows a strict, ordered hierarchy. For every field, sources are consulted in this order and the first valid, non-empty result wins: (1) Platform Adapter, (2) intercepted JSON API payload, (3) JSON-LD structured data, (4) hydrated JS state (`__NEXT_DATA__` / `__NUXT_DATA__`), (5) DOM selector from adapter defaults, (6) LLM fallback. No field is ever resolved by consulting multiple sources and scoring them against each other.
- **INV-EXTRACT-02:** There is no heuristic candidate scoring. A field either resolves at a hierarchy step or it does not. The source that resolved a field is recorded in the record's `source_trace` for diagnostics, but that trace is never used to override or re-rank the result.
- **INV-EXTRACT-03:** Browser pre-capture expansion is generic and unconditional. When a Playwright session is active, all interactive elements matching `details > summary`, `[aria-expanded="false"]`, `button[data-toggle]`, and `[role="tab"]` are clicked and a fixed settle wait is applied before the DOM is captured. The set of requested output fields must not influence which DOM elements are expanded.
- **INV-EXTRACT-04:** Platform Adapters are the only mechanism for site-specific extraction logic. No per-domain extraction rules may exist outside of a versioned, code-backed Adapter module. Runtime-editable selector databases and domain-keyed XPath stores are not permitted.
- **INV-EXTRACT-05:** A listing page that yields zero item records must result in status `listing_detection_failed`. It must never silently fall back to detail-style extraction of the listing page itself.
...
|---|---|---|---|---|
| <span id="oi-01">OI-01</span> | Data retention policy | To be decided | Open | Until resolved, retention is indefinite; manual clear is the only mechanism |
