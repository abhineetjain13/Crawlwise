...
Playwright fallback), LLM-assisted field normalization, and handles two primary extraction
surfaces: ecommerce PDPs/listings and job listings/detail pages.

Pipeline stages: ACQUIRE → EXTRACT → UNIFY → PUBLISH.
Extraction source hierarchy (per INVARIANTS.md §6): adapter → XHR/JSON → JSON-LD →
hydrated state (e.g. __NEXT_DATA__) → DOM → LLM fallback.
Current code is first-match; quality-scorer backfill is a planned Phase 5 item.
...
- Site-specific conditional branches (`if "amazon" in url`, `if "linkedin"`) inside
  generic pipeline functions — INVARIANTS.md §29 forbids tenant/site hardcodes.
...
- Blocking I/O in async code paths (INVARIANTS.md §2: all sync HTTP must use
  `asyncio.to_thread()`; §5: BeautifulSoup in async hot paths must use off-thread helpers).
...
- **Surface bleed:** Are ecommerce fields (brand, sku, color, currency, specifications)
  ...
  path. INVARIANTS.md §13 forbids cross-surface pollution.
...
- **Hydrated state extraction:** Is `__NEXT_DATA__` / `window.__PRELOADED_STATE__` /
  `__APOLLO_STATE__` extraction implemented as a first-class source in the hierarchy,
  or is it absent/bolted-on? (This is the highest-yield signal on modern SPAs.)
- **XHR/API interception:** Is Playwright request interception used to capture background
  JSON API calls before DOM rendering completes? Or does the pipeline rely solely on
  post-render DOM scraping?
- **Source ranking integrity:** Does the pipeline enforce first-match-wins with early
  exit per INVARIANTS.md §6, or does it collect from all sources and then
...
- **LLM boundary:** Is the LLM used as a non-deterministic cleanup layer only, or is it
  ...
  INVARIANTS.md §26 requires LLM config to be snapshot-stable within a run.
...
- Is `advanced_mode` (paginate/scroll/load_more) cleanly separated from browser
  rendering escalation per INVARIANTS.md §18? Or does escalation accidentally trigger
  traversal behavior?
...
- Missing retry logic on network I/O that will cause single-request failures to propagate
  as job failures. (INVARIANTS.md §27: no retry/backoff on 429 — fail fast.)
...
- Modules that are tested via private-function imports — these block structural
  refactoring.
- INVARIANTS.md §29: flag any deleted subsystems (site memory, selector CRUD, discovery
  manifests) that have been resurrected or partially reintroduced.
...
- Reference the relevant source-hierarchy slot from INVARIANTS.md §6 (adapter / XHR/JSON
  / JSON-LD / hydrated state / DOM / LLM) to show where it would slot in.
...
Slot: hydrated-state source (currently absent or incomplete).
...
Constraint: must not violate INVARIANTS.md §16 (user controls not rewritten) or §3
(pipeline config is single source of truth for selectors).
...
- INVARIANTS.md §29 is absolute: generic crawler paths must stay generic.
  No tenant/site hardcodes. Platform behavior is family-based.
