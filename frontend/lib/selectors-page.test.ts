...
describe("selectors page helpers", () => {
  it("infers job detail when fields or URL are job-oriented", () => {
    expect(inferSelectorSurface(["company", "location"], "https://example.com/jobs/123")).toBe("job_detail");
    expect(inferSelectorSurface(["price"], "https://example.com/products/widget")).toBe("ecommerce_detail");
  });
  ...
  it("prefers exact-surface selector records over generic fallbacks", () => {
    ...
    expect(records.map((record) => `${record.surface}:${record.field_name}:${record.css_selector}`)).toEqual([
      "ecommerce_detail:price:.detail-price",
      "generic:price:.generic-price",
    ]);
  });
