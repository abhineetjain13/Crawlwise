...
import { InlineAlert, MutedPanelMessage, PageHeader, SectionCard, SurfaceSection } from "../../../components/ui/patterns";
          ...
          >
            {costLog.length ? (
                ...
                <table className="w-full text-left border-collapse table-fixed min-w-[850px]">
                  <thead>
                    <tr className="border-b border-divider/50">
                      <th className="w-[140px] px-0 py-3 text-[11px] font-bold uppercase tracking-[0.1em] text-muted">Usage & Cost</th>
                      <th className="w-[180px] px-4 py-3 text-[11px] font-bold uppercase tracking-[0.1em] text-muted">Task Type</th>
                      <th className="w-[200px] px-4 py-3 text-[11px] font-bold uppercase tracking-[0.1em] text-muted">Target Entity</th>
                      <th className="px-4 py-3 text-[11px] font-bold uppercase tracking-[0.1em] text-muted">Provider / Model</th>
                      <th className="w-[90px] px-0 py-3 text-[11px] font-bold uppercase tracking-[0.1em] text-muted text-right">Time</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-divider/20">
                    {costLog.slice(0, 40).map((entry) => {
                      ...
                      return (
                        <tr key={entry.id} className="hover:bg-accent/[0.04] transition-colors group">
                          <td className="px-0 py-4">
                            <div className="flex flex-col">
                              <div className="flex items-baseline gap-1.5">
                                <span className="text-sm font-medium tabular-nums text-foreground leading-none">
                                  {totalTokens.toLocaleString()}
                                </span>
                                <span className="text-[11px] font-normal text-muted">tokens</span>
                              </div>
                              <span className="mt-1 text-[12px] font-mono font-medium text-accent leading-none">
                          ...
                          <td className="px-4 py-4">
                            <div className="truncate text-sm font-normal text-foreground/80" title={entry.domain || `Run #${entry.run_id}`}>
                              {entry.domain || (entry.run_id ? `Run #${entry.run_id}` : 'system')}
                            </div>
                          </td>
                          <td className="px-4 py-4">
                            <div className="flex flex-col leading-tight overflow-hidden">
                              <span className="text-sm font-medium text-foreground truncate">{entry.provider}</span>
                              <span className="text-sm font-normal text-muted truncate" title={entry.model}>{entry.model}</span>
                            </div>
                          </td>
                          <td className="px-0 py-4 text-right">
                            <span className="text-sm font-normal text-muted group-hover:text-foreground transition-colors">
              ...
              <div className="p-12 text-center">
                <MutedPanelMessage title="No cost events" description="Detailed LLM usage and token metrics will appear here once active." />
              </div>
            )}
          </SectionCard>
        </div>
