                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {config.is_active ? (
                            <div className="inline-flex items-center gap-1 rounded-full bg-[var(--success-soft)] px-2 py-1 text-[11px] font-semibold text-[var(--success)]">
                              <CheckCircle2 className="size-3.5" />
                              active
                            </div>
                          ) : null}
                        </div>
                        <div className="text-xs text-muted">{config.provider} · {config.model}</div>
                        <div className="text-xs text-muted">{config.api_key_set ? config.api_key_masked : "env-backed or unset"}</div>
                      </div>
            ...
            {costLog.length ? (
              <div className="space-y-2">
                {costLog.slice(0, 8).map((entry) => (
                  <div key={entry.id} className="rounded-[var(--radius-md)] border border-border bg-background-elevated px-3 py-2 text-xs text-muted">
