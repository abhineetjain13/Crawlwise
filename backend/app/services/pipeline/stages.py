...
class ListingBrowserRetryStage:
    ...
    async def execute(self, ctx: PipelineContext) -> None:
        ...
        if acq.method != "curl_cffi":
            return
        child_listing_url = (
            discover_child_listing_candidate_from_soup(ctx.soup, page_url=ctx.url)
            if ctx.soup is not None
            else discover_child_listing_candidate(acq.html or "", page_url=ctx.url)
        )
        category_tile_listing = looks_like_category_tile_listing(ctx.records)
