...
def detect_blocked_page(html: str) -> BlockedPageResult:
    ...
    if (
        "generated by cloudfront" in visible
        and "request blocked" in visible
        and "request could not be satisfied" in visible
    ):
        return BlockedPageResult(
            is_blocked=True,
            reason="blocked_phrase:cloudfront_request_blocked",
            provider="cloudfront",
        )

    if (
        ("403 forbidden" in visible or "403 forbidden" in html_lower or "access denied" in visible)
        and text_len < 500
        and link_count < 3
    ):
        return BlockedPageResult(
